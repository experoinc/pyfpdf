...
class FPDF(object):
    ...
    @check_page
    def multi_cell(self, w, h, txt='', border=0, align='J', fill=0, split_only=False, in_table=False, max_lines=None):
                ...
                if(sep==-1):
                    ...
                    if(self.ws>0):
                        ...
                        if not split_only:
                            self._out('0 Tw')
                    if not split_only:
                        if max_lines is None or nl <= max_lines:
                            next_line = 2 if nl < max_lines else 0
                            self.cell(w,h,substr(s,j,i-j),b,next_line,align,fill)
                    else:
                        ret.append(substr(s,j,i-j))
                else:
                    if(align=='J'):
                    ...
                    if not split_only:
                        if max_lines is None or nl <= max_lines:
                            next_line = 2 if nl < max_lines else 0
                            self.cell(w,h,substr(s,j,sep-j),b,next_line,align,fill)
                    else:
        ...
        if not split_only:
            if max_lines is None or nl <= max_lines:
                if in_table:
                    self.cell(w,h,substr(s,j,i-j),b,0,align,fill)
                else:
                    self.cell(w, h, substr(s, j, i - j), b, 2, align, fill)
                    self.x=self.l_margin
        else:
